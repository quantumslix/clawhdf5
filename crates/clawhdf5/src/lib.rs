...
//!
//! This crate provides an ergonomic interface on top of `clawhdf5-format`.
//!
//! # Reading
//!
//! ```no_run
...
mod tests {
    ...
    fn file_builder_write_to_disk() {
        let dir = std::env::temp_dir();
        let path = dir.join("clawhdf5_test_write.h5");
    ...
    fn error_display() {
        ...
        let err = Error::Format(clawhdf5_format::error::FormatError::SignatureNotFound);
